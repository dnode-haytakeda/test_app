...
- [第 4 部 — Terraform: インフラをコードで管理する](#第-4-部--terraform-インフラをコードで管理する) ← ⭐ 実装開始
- [第 5 部 — AWS リソース設計: アプリをどこに置くか](#第-5-部--aws-リソース設計-アプリをどこに置くか) ← ⭐ Terraform モジュール作成
- [第 6 部 — GitHub Actions の基礎](#第-6-部--github-actions-の基礎)
- [第 7 部 — CI パイプライン: 品質を守る自動化](#第-7-部--ci-パイプライン-品質を守る自動化) ← ⭐ ci.yml 作成
- [第 8 部 — CD パイプライン: AWS へ自動デプロイ](#第-8-部--cd-パイプライン-aws-へ自動デプロイ) ← ⭐ deploy.yml 作成
...
## 🗺️ 実行計画 — 作成するファイル一覧とディレクトリ構成

> **このセクションが「手順書」としての地図です。** 以降の各 Part は、ここに書かれたファイルの中身と「なぜそう書くのか」を詳しく解説しています。

### 最終的に作成するディレクトリ構成

```
test_app/                              ← プロジェクトルート (既存)
├── backend/                           ← 既存
│   ├── Dockerfile                     ← 既存 (開発用)
│   └── Dockerfile.prod                ← 【新規作成】本番用 Dockerfile
├── frontend/                          ← 既存
│   └── Dockerfile                     ← 既存
├── infrastructure/                    ← 【新規ディレクトリ】Terraform コード一式
│   ├── modules/                       ← 再利用可能なインフラ部品
│   │   ├── vpc/
│   │   │   ├── main.tf               ← VPC / サブネット / NAT Gateway
│   │   │   ├── variables.tf          ← 入力パラメータ定義
│   │   │   └── outputs.tf            ← 他モジュールへの出力値
│   │   ├── ecr/
│   │   │   ├── main.tf               ← コンテナレジストリ + ライフサイクル
│   │   │   ├── variables.tf
│   │   │   └── outputs.tf
│   │   ├── rds/
│   │   │   ├── main.tf               ← PostgreSQL + セキュリティグループ
│   │   │   ├── variables.tf
│   │   │   └── outputs.tf
│   │   ├── alb/
│   │   │   ├── main.tf               ← ロードバランサー + ターゲットグループ
│   │   │   ├── variables.tf
│   │   │   └── outputs.tf
│   │   ├── ecs/
│   │   │   ├── main.tf               ← Fargate クラスタ + サービス + タスク定義
│   │   │   ├── variables.tf
│   │   │   └── outputs.tf
│   │   ├── s3-cloudfront/
│   │   │   ├── main.tf               ← フロントエンド配信 (S3 + CDN)
│   │   │   ├── variables.tf
│   │   │   └── outputs.tf
│   │   └── iam/
│   │       ├── main.tf               ← OIDC プロバイダ + デプロイ用ロール
│   │       ├── variables.tf
│   │       └── outputs.tf
│   └── environments/
│       ├── staging/
│       │   ├── main.tf               ← staging 環境の構成
│       │   ├── variables.tf
│       │   ├── outputs.tf
│       │   ├── terraform.tfvars       ← staging のパラメータ値
│       │   └── backend.tf             ← S3 リモート state 設定
│       └── production/
│           ├── main.tf               ← production 環境の構成
│           ├── variables.tf
│           ├── outputs.tf
│           ├── terraform.tfvars       ← production のパラメータ値
│           └── backend.tf             ← S3 リモート state 設定
├── .github/
│   └── workflows/
│       ├── ci.yml                     ← 【新規作成】CI パイプライン
│       └── deploy.yml                 ← 【新規作成】CD パイプライン
└── docker-compose.yml                 ← 既存
```

### 作成するファイルの一覧と順序

以下の表が「何を・どこに・どの順序で」作るかの完全なリストです。**上から順に進めてください。**

#### Phase 1: 本番用 Docker イメージの準備

| # | ファイルパス | 内容 | 解説セクション |
|---|------------|------|--------------|
| 1 | `backend/Dockerfile.prod` | マルチステージビルドの本番用 Dockerfile (Gunicorn + Uvicorn) | [5.5](#55-本番用-dockerfileマルチステージビルド) |

#### Phase 2: Terraform — インフラ基盤の構築

> ⚠️ Phase 2 に入る前に: AWS アカウント作成、AWS CLI 設定 (`aws configure`)、Terraform インストール を完了させること。

| # | ファイルパス | 内容 | 解説セクション |
|---|------------|------|--------------|
| 2 | `infrastructure/modules/vpc/variables.tf` | `project_name`, `vpc_cidr`, `availability_zones` の定義 | [5.2](#52-vpcの構築terraform) |
| 3 | `infrastructure/modules/vpc/main.tf` | VPC, サブネット (public/private/db), IGW, NAT GW, ルートテーブル | [5.2](#52-vpcの構築terraform) |
| 4 | `infrastructure/modules/vpc/outputs.tf` | `vpc_id`, `public_subnet_ids`, `private_subnet_ids`, `database_subnet_ids` | [5.2](#52-vpcの構築terraform) |
| 5 | `infrastructure/modules/ecr/variables.tf` | `project_name` | [5.4](#54-ecrコンテナレジストリの構築) |
| 6 | `infrastructure/modules/ecr/main.tf` | ECR リポジトリ (IMMUTABLE タグ) + ライフサイクルポリシー | [5.4](#54-ecrコンテナレジストリの構築) |
| 7 | `infrastructure/modules/ecr/outputs.tf` | `repository_url`, `repository_arn` | [5.4](#54-ecrコンテナレジストリの構築) |
| 8 | `infrastructure/modules/rds/variables.tf` | `project_name`, `vpc_id`, `database_subnet_ids`, `ecs_security_group_id`, DB パラメータ | [5.3](#53-rdsデータベースの構築) |
| 9 | `infrastructure/modules/rds/main.tf` | RDS インスタンス + サブネットグループ + セキュリティグループ | [5.3](#53-rdsデータベースの構築) |
| 10 | `infrastructure/modules/rds/outputs.tf` | `db_host`, `db_port`, `db_secret_arn` | [5.3](#53-rdsデータベースの構築) |
| 11 | `infrastructure/modules/alb/variables.tf` | `project_name`, `vpc_id`, `public_subnet_ids`, `certificate_arn` | [5.9](#59-セキュリティグループ--多層防御) |
| 12 | `infrastructure/modules/alb/main.tf` | ALB + リスナー + ターゲットグループ + セキュリティグループ | [5.9](#59-セキュリティグループ--多層防御) |
| 13 | `infrastructure/modules/alb/outputs.tf` | `alb_dns_name`, `target_group_arn`, `alb_security_group_id` | [5.9](#59-セキュリティグループ--多層防御) |
| 14 | `infrastructure/modules/ecs/variables.tf` | `project_name`, `vpc_id`, `private_subnet_ids`, `ecr_repository_url`, 他 | [5.6](#56-ecs-fargate-の構築) |
| 15 | `infrastructure/modules/ecs/main.tf` | ECS クラスタ + タスク定義 + サービス + Auto Scaling + IAM ロール | [5.6](#56-ecs-fargate-の構築) |
| 16 | `infrastructure/modules/ecs/outputs.tf` | `cluster_name`, `service_name`, `ecs_security_group_id` | [5.6](#56-ecs-fargate-の構築) |
| 17 | `infrastructure/modules/s3-cloudfront/variables.tf` | `project_name`, `domain_name`, `certificate_arn` | [5.7](#57-s3--cloudfrontフロントエンド配信) |
| 18 | `infrastructure/modules/s3-cloudfront/main.tf` | S3 バケット + CloudFront + OAC + SPA フォールバック | [5.7](#57-s3--cloudfrontフロントエンド配信) |
| 19 | `infrastructure/modules/s3-cloudfront/outputs.tf` | `s3_bucket_name`, `cloudfront_distribution_id`, `cloudfront_domain` | [5.7](#57-s3--cloudfrontフロントエンド配信) |
| 20 | `infrastructure/modules/iam/variables.tf` | `project_name`, `github_org`, `github_repo` | [2.4](#24-cicd-における-iam--github-actions-が-aws-を操作するには) |
| 21 | `infrastructure/modules/iam/main.tf` | OIDC プロバイダ + GitHub Actions 用 IAM ロール + ポリシー | [2.4](#24-cicd-における-iam--github-actions-が-aws-を操作するには) |
| 22 | `infrastructure/modules/iam/outputs.tf` | `deploy_role_arn` | [2.4](#24-cicd-における-iam--github-actions-が-aws-を操作するには) |

#### Phase 3: Terraform — 環境別の構成

| # | ファイルパス | 内容 | 解説セクション |
|---|------------|------|--------------|
| 23 | `infrastructure/environments/production/backend.tf` | S3 + DynamoDB リモート state 設定 | [4.5](#45-state状態管理) |
| 24 | `infrastructure/environments/production/variables.tf` | 全モジュール共通の変数定義 | [4.3](#43-terraform-の基本概念) |
| 25 | `infrastructure/environments/production/terraform.tfvars` | 本番環境の具体的なパラメータ値 | [4.3](#43-terraform-の基本概念) |
| 26 | `infrastructure/environments/production/main.tf` | 全モジュールの呼び出し・接続 | [4.6](#46-モジュール--コードの再利用) |
| 27 | `infrastructure/environments/production/outputs.tf` | デプロイに必要な出力値 | [4.3](#43-terraform-の基本概念) |

#### Phase 4: GitHub Actions — CI/CD パイプライン

| # | ファイルパス | 内容 | 解説セクション |
|---|------------|------|--------------|
| 28 | `.github/workflows/ci.yml` | lint + test + build + compose-smoke | [7.1](#71-ci-パイプラインの全体像)〜[7.4](#74-compose-smoke-テスト) |
| 29 | `.github/workflows/deploy.yml` | OIDC 認証 → ECR Push → Migration → ECS 更新 → S3 同期 | [8.2](#82-完全な-cd-ワークフロー) |

#### Phase 5: GitHub リポジトリの設定 (GUI 操作)

| # | 設定場所 | 内容 | 解説セクション |
|---|---------|------|--------------|
| 30 | Settings → Secrets | `AWS_DEPLOY_ROLE_ARN`, `CLOUDFRONT_DISTRIBUTION_ID` を登録 | [6.4](#64-環境変数と-secrets) |
| 31 | Settings → Environments | `production` 環境を作成、Protection Rules を設定 | [8.4](#84-github-environments--デプロイの安全装置) |
| 32 | Settings → Branch protection | `main` ブランチの保護ルールを設定 | [9.1](#91-branch-protection-rules--main-を守る城壁) |
...
| ファイル | 一言で言うと | 行数目安 |
|---------|------------|---------|
| `Dockerfile.prod` | マルチステージビルド。Stage1=依存インストール、Stage2=本番実行 | ~40行 |
| `modules/*/variables.tf` | そのモジュールが受け取るパラメータの型と説明 | ~20-40行 |
| `modules/*/main.tf` | AWS リソースの実体定義 (resource ブロック) | ~50-200行 |
| `modules/*/outputs.tf` | 他モジュールに渡す値の定義 | ~10-20行 |
| `environments/*/main.tf` | module ブロックで各モジュールを呼び出し、パラメータを渡す | ~80行 |
| `environments/*/backend.tf` | Terraform state の保存先 (S3) を定義 | ~15行 |
| `environments/*/terraform.tfvars` | 具体的な値 (`project_name = "test-app"` 等) | ~20行 |
| `.github/workflows/ci.yml` | トリガー → lint → test → build → smoke テスト | ~120行 |
| `.github/workflows/deploy.yml` | トリガー → テスト → AWS認証 → ビルド → デプロイ | ~180行 |

> 💡 **迷ったらこのセクションに戻ってきてください。** 以降の各 Part は、上記ファイルの中身を一つずつ詳しく解説しています。Part の番号 = Phase の番号ではありません。Part 1〜3 は「知識編」、Part 4〜9 が「実装編」です。
...
## 🔧 事前準備 — AWS アカウントと Terraform State のセットアップ

> **ファイル作成なし。AWS コンソールとターミナルでの操作のみ。**
> 本書のステップに入る前に、以下を完了させてください。

### 準備 1: AWS アカウント作成 & MFA 有効化

> 🖥️ **AWS コンソール操作**

1. [aws.amazon.com](https://aws.amazon.com/) → 「無料アカウントを作成」
2. メールアドレス・パスワード・アカウント名を入力
3. 支払い情報を登録（Free Tier 内なら課金されない）
4. 電話番号認証 → アカウント作成完了

**MFA の有効化（必須）:**

5. AWS コンソールにログイン
6. 右上のアカウント名 → 「セキュリティ認証情報」
7. 「MFA デバイスの割り当て」→ 「MFA デバイスを割り当て」
8. 「認証アプリケーション」を選択 → 次へ
9. Google Authenticator / 1Password 等でQRコードをスキャン
10. 確認コードを 2 回入力 → 完了

> ⚠️ **MFA なしで本番 AWS を使うのは、家の鍵を開けっ放しにするのと同じです。**

### 準備 2: IAM ユーザーの作成

> 🖥️ **AWS コンソール操作**

1. AWS コンソール → 「IAM」→ 左メニュー「ユーザー」
2. 「ユーザーを作成」
3. ユーザー名: `admin`（任意）
4. 「AWS マネジメントコンソールへのユーザーアクセスを提供する」にチェック
5. 「ユーザーにポリシーを直接アタッチする」→ `AdministratorAccess` を選択
6. 「ユーザーの作成」→ ログイン URL・パスワードを控える
7. **以後はこの IAM ユーザーでログインする**（ルートアカウントは使わない）
...
# 設定
aws configure
# AWS Access Key ID: （IAM ユーザーのアクセスキーを入力）
...
# → "Account": "123456789012" のように表示されればOK
```

> 💡 **アクセスキーの取得**: IAM → ユーザー → `admin` → 「セキュリティ認証情報」→「アクセスキーを作成」→「CLI」を選択。
...
### 準備 6: Terraform State 管理用 S3 バケットの手動作成

> 🖥️ **AWS コンソール操作**（Terraform 自身の state を Terraform で管理する「鶏と卵」問題を避けるため、ここだけは手動）

**S3 バケット作成:**
1. AWS コンソール → S3 → 「バケットを作成」
2. バケット名: `test-app-terraform-state-{あなたのアカウントID}`（グローバルに一意）
3. リージョン: `ap-northeast-1`（東京）
4. 「バージョニング」: **有効**
5. 「デフォルトの暗号化」: **SSE-S3 を有効**
6. 「パブリックアクセスをすべてブロック」: **有効のまま**
7. 「バケットを作成」

**DynamoDB テーブル作成（同時実行防止用）:**
1. AWS コンソール → DynamoDB → 「テーブルを作成」
2. テーブル名: `test-app-terraform-lock`
3. パーティションキー: `LockID`（文字列型）
4. その他はデフォルトのまま → 「テーブルの作成」
...
> # エラーなく空の結果が返ればOK
>
> aws dynamodb describe-table --table-name test-app-terraform-lock --region ap-northeast-1 --query 'Table.TableStatus'
> # "ACTIVE" が返ればOK
> ```
...
> 📄 **書き込み先ファイル**: `infrastructure/modules/iam/main.tf`（新規作成）
...
> | ファイル | 内容 |
> |------|------|
> | `infrastructure/environments/production/backend.tf` | S3 リモート state 設定 |
> | `infrastructure/environments/production/main.tf` | 全モジュールの呼び出し |
> | `infrastructure/environments/production/variables.tf` | 全変数の定義 |
> | `infrastructure/environments/production/terraform.tfvars` | 具体的なパラメータ値 |
> | `infrastructure/environments/production/outputs.tf` | 出力値定義 |
>
> **コマンド実行:**
> ```bash
> cd infrastructure/environments/production
> terraform init      # 初回のみ
> terraform plan      # 差分確認 (必ず実行)
...
> 📄 **書き込み先ファイル**: `infrastructure/environments/production/backend.tf`（新規作成）
...
> 📄 **書き込み先ファイル**: `infrastructure/environments/production/main.tf`（新規作成）
...
> | ファイル | 内容 |
> |------|------|
> | `backend/Dockerfile.prod` | 本番用マルチステージ Dockerfile |
> | `infrastructure/modules/vpc/main.tf` | VPC + サブネット + NAT GW |
> | `infrastructure/modules/vpc/variables.tf` | VPC モジュールの入力パラメータ |
> | `infrastructure/modules/vpc/outputs.tf` | VPC モジュールの出力値 |
> | `infrastructure/modules/rds/main.tf` | RDS インスタンス + SG |
> | `infrastructure/modules/ecr/main.tf` | ECR リポジトリ + ライフサイクル |
> | `infrastructure/modules/ecs/main.tf` | ECS クラスタ + サービス + タスク定義 |
> | `infrastructure/modules/alb/main.tf` | ALB + ターゲットグループ |
> | `infrastructure/modules/s3-cloudfront/main.tf` | S3 + CloudFront + OAC |
...
> 📄 **書き込み先ファイル**: `infrastructure/modules/vpc/main.tf`（新規作成）
...
> 📄 **書き込み先ファイル**: `infrastructure/modules/rds/main.tf`（新規作成）
...
> 📄 **書き込み先ファイル**: `infrastructure/modules/ecr/main.tf`（新規作成）
...
> 📄 **書き込み先ファイル**: `backend/Dockerfile.prod`（新規作成）
...
> 📄 **書き込み先ファイル**: `infrastructure/modules/ecs/main.tf`（新規作成）
...
> 📄 **書き込み先ファイル**: `infrastructure/modules/s3-cloudfront/main.tf`（新規作成）
...
> 📄 **書き込み先ファイル**: `infrastructure/modules/s3-cloudfront/main.tf`（追記）
...
> 📄 **書き込み先ファイル**: `infrastructure/modules/ecs/main.tf`（追記、セキュリティグループ部分）
...
> 📄 **書き込み先ファイル**: `.github/workflows/reusable-docker-build.yml`（新規作成）
...
> 📄 **書き込み先ファイル**: `.github/workflows/deploy.yml`（呼び出し側の例）
...
> | ファイル | 内容 |
> |------|------|
> | `.github/workflows/ci.yml` | CI パイプライン (lint + test + build + smoke) |
...
> 📄 **書き込み先ファイル**: `.github/workflows/ci.yml`（以下 7.2〜7.4 のスニペットをこのファイルにまとめる）
...
> | ファイル | 内容 |
> |------|------|
> | `.github/workflows/deploy.yml` | CD パイプライン (OIDC認証 → ECR Push → Migration → ECS更新 → S3同期) |
>
> **GitHub での GUI 設定:**
> | 設定場所 | 内容 |
> |---------|------|
> | Settings → Secrets | `AWS_DEPLOY_ROLE_ARN`, `CLOUDFRONT_DISTRIBUTION_ID` |
> | Settings → Environments | `production` 環境 + Protection Rules |
> | Settings → Branches | `main` の Branch Protection Rules |
...
> 📄 **書き込み先ファイル**: `.github/workflows/deploy.yml`（新規作成）
...
#### 🖥️ GitHub Settings の GUI 設定手順

以下の 3 つの設定を GitHub リポジトリの Web UI から行います。
...
| Name | Value | 説明 |
|------|-------|------|
| `AWS_DEPLOY_ROLE_ARN` | `arn:aws:iam::<AWSアカウントID>:role/test-app-github-actions` | Part 2 で作成した IAM ロールの ARN |
| `CLOUDFRONT_DISTRIBUTION_ID` | `E1XXXXXXXXXX`（CloudFront コンソールで確認） | フロントエンドキャッシュ無効化に使用 |

> ⚠️ **注意**: Secret の値は登録後に再表示できません。値を間違えた場合は **Update** で上書きしてください。

##### ② Environments の作成と Protection Rules

1. **Settings** タブ → 左メニュー **Environments** をクリック
2. **New environment** をクリック → 名前に `production` と入力 → **Configure environment** をクリック
3. **Environment protection rules** セクションで以下を設定:
   - ✅ **Required reviewers** にチェック → 承認者（自分のアカウント）を追加
   - ✅ **Wait timer** に `5`（分）を入力（任意）
4. **Deployment branches and tags** セクションで:
   - ドロップダウンから **Selected branches and tags** を選択
   - **Add deployment branch or tag rule** をクリック
   - `main` と入力して追加
5. **Save protection rules** をクリック

> 💡 これにより、`main` ブランチ以外からの本番デプロイが禁止され、デプロイ時に承認が必要になります。

##### ③ Branch Protection Rules の設定

1. **Settings** タブ → 左メニュー **Branches** をクリック
2. **Add branch protection rule** をクリック
3. **Branch name pattern** に `main` と入力
4. 以下にチェックを入れる:
   - ✅ **Require a pull request before merging**
     - ✅ **Require approvals**: 1（最低 1 人のレビュー承認が必要）
   - ✅ **Require status checks to pass before merging**
     - 検索ボックスで以下の CI Job を追加:
       - `backend-lint`
       - `backend-test`
       - `frontend-lint`
       - `frontend-build`
...
> 💡 これにより、CI が通らない PR は `main` にマージできなくなります。「CI がグリーンでないコードは本番に行かない」を仕組みで保証します。
...
> 📄 **書き込み先ファイル**: `infrastructure/modules/monitoring/main.tf`（新規作成）
...
> 📄 **書き込み先ファイル**: `infrastructure/modules/waf/main.tf`（新規作成）
